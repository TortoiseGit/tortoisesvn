...
<xsl:variable name="xnavigation">
  ...
  <xsl:call-template name="toHex">
    <xsl:with-param name="n" select="9504 + $htmlhelp.show.menu * 65536                                           + $htmlhelp.show.advanced.search * 131072                                           + $htmlhelp.show.favorities * 4096"/>
  </xsl:call-template>
</xsl:variable>
...
<xsl:template name="hhk">
  <xsl:call-template name="write.text.chunk">
    ...
    <xsl:with-param name="method" select="'text'"/>
    <xsl:with-param name="content"><xsl:text disable-output-escaping="yes">&lt;!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML//EN"&gt;
&lt;HTML&gt;
&lt;HEAD&gt;
...
<xsl:template name="write.indexterm.hhk">
  ...
  <xsl:text disable-output-escaping="yes">&lt;LI&gt; &lt;OBJECT type="text/sitemap"&gt;
    &lt;param name="Name" value="</xsl:text><xsl:value-of select="$text.escaped"/><xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
      <xsl:if test="not(seealso)">
        ...
        <xsl:text disable-output-escaping="yes">&lt;param name="Name" value="</xsl:text>
          <xsl:value-of select="$title"/>
        <xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
        <xsl:text disable-output-escaping="yes">&lt;param name="Local" value="</xsl:text>
          <xsl:value-of select="$href"/>
        <xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
      </xsl:if>
      <xsl:if test="seealso">
        <xsl:text disable-output-escaping="yes">&lt;param name="See Also" value="</xsl:text>

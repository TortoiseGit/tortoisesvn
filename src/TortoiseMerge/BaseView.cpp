...
{
    static viewdata emptyLine;
    if (emptyLine.ending != EOL_NOENDING)
    {
        emptyLine.ending = EOL_NOENDING;
        emptyLine.hidestate = HIDESTATE_SHOWN;
        emptyLine.linenumber = -1;
        emptyLine.movedIndex = -1;
        emptyLine.sLine = _T("");
        emptyLine.state = DIFFSTATE_EMPTY;
    }
    return emptyLine;
}

...
public:
    ...
    UnicodeType GetUnicodeType() const { return m_saveParams.unicodeType; }
    EOL         GetLineEndings() const { return m_saveParams.lineEndings; }

    void Add(const CString& sLine, EOL ending)
    {
        CFileTextLine temp = {sLine, ending};
        CStdFileLineArray::Add(temp);
    }
    void InsertAt(int index, const CString& strVal, EOL ending)
    {
        CFileTextLine temp = {strVal, ending};
        CStdFileLineArray::InsertAt(index, temp);
    }
...
private:
     ...
     */
    static UnicodeType CheckUnicodeType(LPVOID pBuffer, int cb);

    void SetErrorString();

    static void StripWhiteSpace(CString& sLine, DWORD dwIgnoreWhitespaces, bool blame);
    bool        StripComments(CString& sLine, bool bInBlockComment) const;
    static bool IsInsideString(const CString& sLine, int pos);
...
public:
    ...
    CBuffer(const CBuffer* const src)
    {
        Init();
        Copy(*src);
    }
...
private:
    ...
    void Free() const { delete[] m_pBuffer; }
    void Init()
    {
        m_pBuffer    = nullptr;
        m_nUsed      = 0;
...
public:
    ...
    void                   Write(const CBuffer& buffer);                 ///< write preencoded buffer
...
{
public:
    CUtf16BeFilter(CStdioFile* pFile)
...
{
public:
    CUtf32BeFilter(CStdioFile* pFile)

...
static void usage(void)
{
    ...
    j += sprintf(buf+j, "  -V        print version information\n");
...
int main(int argc, char **argv)
{
	...
	if (back == NULL) {
	    mboxprintf("Internal fault: Unsupported protocol found\n");
	    return 1;
	}
    }
	...
	mboxprintf("Plink requires WinSock 2\n");
	return 1;
    }
	...
	if (error) {
	    mboxprintf("Unable to open connection:\n%s", error);
	    return 1;
	}
	...
	mboxprintf("Unable to create output thread\n");
	cleanup_exit(1);
    }
	...
	mboxprintf("Unable to create error output thread\n");
	cleanup_exit(1);
    }
	...
	if (!sending && back->sendok(backhandle)) {
	    ...
	    if (!CreateThread(NULL, 0, stdin_read_thread,
			      &idata, 0, &in_threadid)) {
		mboxprintf("Unable to create input thread\n");
		cleanup_exit(1);
	    }
	    sending = TRUE;
	}
    ...
    if (exitcode < 0) {
	mboxprintf("Remote process exit code unavailable\n");

...
BOOL EditPropUserState::OnInitDialog()
{
    ...
    if (m_userprop->stateEntries.size() <= 3)
    {
        ...
        for (auto it = m_userprop->stateEntries.cbegin(); it != m_userprop->stateEntries.cend(); ++it)
        {
            ...
            case 0:     // IDC_RADIO1
                ...
                SetDlgItemText(IDC_RADIO1, it->second);
                AdjustControlSize(IDC_RADIO1);
                if (m_PropValue.compare(CUnicodeUtils::GetUTF8(it->first))==0)
                {
                ...
                if (!checked)
                {
                    if (it->first.Compare(m_userprop->stateDefaultVal)==0)
                        CheckRadioButton(IDC_RADIO1, IDC_RADIO3, IDC_RADIO1);
                }
                break;
            case 1:     // IDC_RADIO2
                GetDlgItem(IDC_RADIO2)->ShowWindow(SW_SHOW);
                SetDlgItemText(IDC_RADIO2, it->second);
            ...
            case 2:     // IDC_RADIO3
                ...
                SetDlgItemText(IDC_RADIO3, it->second);
    ...
    else
    {
        ...
        for (auto it = m_userprop->stateEntries.cbegin(); it != m_userprop->stateEntries.cend(); ++it)
        {
            int index = m_combo.AddString(it->second);
            m_combo.SetItemData(index, (DWORD_PTR)&it->first);
            if (m_PropValue.compare(CUnicodeUtils::GetUTF8(it->first))==0)
                selString = it->second;
            if (it->first.Compare(m_userprop->stateDefaultVal)==0)
                selDefString = it->second;
        }
    ...
    if (m_userprop->stateEntries.size() <= 3)
    {
        ...
        for (auto it = m_userprop->stateEntries.cbegin(); it != m_userprop->stateEntries.cend(); ++it)
        {
            ...
            case 0:
                ...
                {
                    m_PropValue = CUnicodeUtils::GetUTF8(it->first);
                    m_bChanged = true;
                }
                break;
            case 1:

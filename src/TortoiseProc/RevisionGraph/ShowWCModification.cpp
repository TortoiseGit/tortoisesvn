...
CShowWCModification::CShowWCModification (CRevisionGraphOptionList& list)
    : inherited (list)
{
}
...
bool CShowWCModification::IsActive() const
{
    return true;
}
...
CShowWCModification::ShallRemove (const CFullGraphNode* node) const
{
    ...
    return node->GetClassification().Is (CNodeClassification::IS_MODIFIED_WC)
         ? IsSelected() ? ICopyFilterOption::PRESERVE_NODE

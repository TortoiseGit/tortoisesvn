...
// Copyright (C) 2003-2008, 2011, 2013-2014, 2018, 2021 - TortoiseSVN
...
virtual void volatile wchar_t while xor xor_eq"));
            SendEditor(SCI_SETKEYWORDS, 3, reinterpret_cast<LPARAM>(L"a addindex addtogroup anchor arg attention \
...
set static super switch this throw true try typeof undefined var void while with"));
            SendEditor(SCI_SETKEYWORDS, 1, reinterpret_cast<LPARAM>(L"Array Arguments Accessibility Boolean Button Camera Color \
...
file hidden image"));
            SendEditor(SCI_SETKEYWORDS, 1, reinterpret_cast<LPARAM>(L"assign audio block break catch choice clear disconnect else elseif \
...
script sentence subdialog submit throw transfer value var voice vxml"));
            SendEditor(SCI_SETKEYWORDS, 2, reinterpret_cast<LPARAM>(L"accept age alphabet anchor application base beep bridge category charset \
...
transferaudio type value variant version volume xml:lang"));
            SendEditor(SCI_SETKEYWORDS, 3, reinterpret_cast<LPARAM>(L"and assert break class continue def del elif \
else except exec finally for from global if import in is lambda None \
not or pass print raise return try while yield"));
            SendEditor(SCI_SETKEYWORDS, 4, reinterpret_cast<LPARAM>(L"and argv as argc break case cfunction class continue declare default do \
...
void TortoiseBlame::SetupCppLexer() const
{

...
void CPicWindow::DrawViewTitle(HDC hDC, RECT * rect)
{
    ...
    HFONT hFont = nullptr;
    hFont = CreateFont(-CDPIAware::Instance().PointsToPixels(*this, pSecondPic ? 9 : 10), 0, 0, 0, FW_DONTCARE, false, false, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, L"Segoe UI");
    ...
    hdc = BeginPaint(hwnd, &ps);
    {
        ...
        else
        {
            ...
            if (GetTextExtentPoint32(memDC, str, (int)wcslen(str), &stringsize))
            {
                ...
                ::DrawText(memDC, str, (int)wcslen(str), &drawRC, DT_HIDEPREFIX | DT_NOPREFIX | DT_SINGLELINE);
            }
            SelectObject(memDC, (HGDIOBJ)hFontOld);
            DeleteObject(hFont);
        }
...
void CPicWindow::SetTheme(bool bDark)
{
    if (bDark)
    {
        ...
        SetClassLongPtr(hwndTrack, GCLP_HBRBACKGROUND, (LONG_PTR)GetStockObject(BLACK_BRUSH));

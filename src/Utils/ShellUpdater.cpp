...
void CShellUpdater::UpdateShell()
{
    ...
    CAutoFile hPipe = CreateFile(GetCacheCommandPipeName(), // pipe name
    ...
    if (SetNamedPipeHandleState(hPipe,         // pipe handle
                                ...
                                nullptr) == 0) // don't set maximum time
    {
    ...
    for (int nPath = 0; nPath < m_pathsForUpdating.GetCount(); nPath++)
    {
        ...
        if (!m_pathsForUpdating[nPath].IsDirectory())
        {
            ...
            SHChangeNotify(SHCNE_UPDATEITEM, SHCNF_PATH | SHCNF_FLUSHNOWAIT, m_pathsForUpdating[nPath].GetWinPath(), nullptr);
        }
...
bool CShellUpdater::RebuildIcons()
{
    ...
    auto lRegResult = RegOpenKeyEx(HKEY_CURRENT_USER, L"Control Panel\\Desktop\\WindowMetrics",

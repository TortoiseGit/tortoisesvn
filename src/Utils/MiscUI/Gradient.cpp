...
void CGradient::Draw(CDC* pDC, const CRect& rect, COLORREF colorStart, COLORREF colorEnd, BOOL bHorz /* = TRUE*/, UINT nSteps /* = 64*/)
{
    ...
    {
        BYTE bR = static_cast<BYTE>((GetRValue(colorStart) * (nSteps - i) + GetRValue(colorEnd) * i) / nSteps);
        BYTE bG = static_cast<BYTE>((GetGValue(colorStart) * (nSteps - i) + GetGValue(colorEnd) * i) / nSteps);
        BYTE bB = static_cast<BYTE>((GetBValue(colorStart) * (nSteps - i) + GetBValue(colorEnd) * i) / nSteps);

...
// Copyright (C) 2003-2006, 2008-2013, 2015, 2020-2021 - TortoiseSVN
...
CStringA CUnicodeUtils::GetUTF8(const CStringW& string)
{
    ...
    int len = WideCharToMultiByte(CP_UTF8, 0, static_cast<const wchar_t*>(string), size, buffer, 4 * size, nullptr, nullptr);
    if (len == 0)
        return CStringA();
...
CString CUnicodeUtils::GetUnicode(const CStringA& string)
{
    ...
    // Note: always use the Windows API function, do NOT try to implement a function that's faster.
    // The API takes this long because it can handle all edge cases, and that's what we need.
    int len = MultiByteToWideChar(CP_UTF8, 0, static_cast<const char*>(string), size, buffer, 2 * size);
    if (len == 0)
        return CString();
...
{
    int           size = static_cast<int>(wide.length());
    CBuffer<char> buffer(4 * size);

    int len = WideCharToMultiByte(CP_UTF8, 0, wide.c_str(), size, buffer, 4 * size, nullptr, nullptr);
    if (len == 0)
        return std::string();
...
{
    int              size = static_cast<int>(utf8.length());
    CBuffer<wchar_t> buffer(2 * size);
...
{
    int              size = static_cast<int>(ansi.length());
    CBuffer<wchar_t> buffer(2 * size);
    ...
    len = WideCharToMultiByte(CP_UTF8, 0, buffer, len, buffer2, 4 * size, nullptr, nullptr);
    if (len == 0)
        return ansi;
...
[[maybe_unused]] static class CUnicodeUtilsTests
{
public:
    CUnicodeUtilsTests()
    {
